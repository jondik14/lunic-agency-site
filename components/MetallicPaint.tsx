...
export interface ParseLogoResult {
  imageData: ImageData;
  pngBlob: Blob;
}

export function parseLogoImage(file: File): Promise<ParseLogoResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      if (file.type === 'image/svg+xml') {
        (img as HTMLImageElement).width = 1000;
        (img as HTMLImageElement).height = 1000;
      }
...
export default function MetallicPaint({ imageData, params: paramsProp }: MetallicPaintProps) {
  ...
  const params = { ...defaultParams, ...paramsProp };

  function updateUniforms() {
    if (!gl || !uniforms.u_edge) return;
    gl.uniform1f(uniforms.u_edge, params.edge ?? 1);
    gl.uniform1f(uniforms.u_patternBlur, params.patternBlur ?? 0.005);
    gl.uniform1f(uniforms.u_patternScale, params.patternScale ?? 2);
    gl.uniform1f(uniforms.u_refraction, params.refraction ?? 0.015);
    gl.uniform1f(uniforms.u_liquid, params.liquid ?? 0.07);
  }
  ...
  useEffect(() => {
    ...
    if (!glCtx.getProgramParameter(program, glCtx.LINK_STATUS)) {
      console.error('Program link:', glCtx.getProgramInfoLog(program));
      return;
    }

    const uniformMap: Record<string, WebGLUniformLocation> = {};
    const n = glCtx.getProgramParameter(program, glCtx.ACTIVE_UNIFORMS);
    for (let i = 0; i < n; i++) {
      const u = glCtx.getActiveUniform(program, i);
      if (u) uniformMap[u.name] = glCtx.getUniformLocation(program, u.name)!;
    }
    setUniforms(uniformMap);

    const vertices = new Float32Array([-1, -1, 1, -1, -1, 1, 1, 1]);
    const vb = glCtx.createBuffer();
    glCtx.bindBuffer(glCtx.ARRAY_BUFFER, vb);
  ...
  useEffect(() => {
    ...
    function render(currentTime: number) {
      ...
      totalAnimationTime.current += delta * (params.speed ?? 0.3);
  ...
  useEffect(() => {
    ...
    if (!canvasEl || !gl || !uniforms.u_img_ratio || !imageData) return;
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('MetallicPaint texture upload:', e);
    }
    return () => {
